...
async def on_mute(client: Client, message: Message):
    ...
    if message.reply_to_message is None:
        # <editor-fold defaultstate="collapsed" desc="logging">
        logger.info(f'[{message.chat.id} ({message.message_id})] Message have no reply_to_message; deleting...')
        # </editor-fold>
    ...
    if message.reply_to_message.from_user is None:
        # <editor-fold defaultstate="collapsed" desc="logging">
        logger.info(f'[{message.chat.id} ({message.message_id})] Replied not to user; deleting...')
        # </editor-fold>
        await message.delete()
        # <editor-fold defaultstate="collapsed" desc="logging">
        logger.info(f'[{message.chat.id} ({message.message_id})] Done.')
    ...
    if match.group('term') is None:
        # <editor-fold defaultstate="collapsed" desc="logging">
        logger.info(f'[{message.chat.id} ({message.message_id})] No term specified; muting for 1 day...')
        # </editor-fold>
        await client.restrict_chat_member(message.chat.id, target.id,
                                          ChatPermissions(can_send_messages=False), round(time.time()) + 86400)
        # <editor-fold defaultstate="collapsed" desc="logging">
        logger.info(f'[{message.chat.id} ({message.message_id})] Successfully muted target;'
                     f'deleting message with command and informing...')
        # </editor-fold>
        await message.delete()
        # <editor-fold defaultstate="collapsed" desc="logging">
        logger.info(f'[{message.chat.id} ({message.message_id})] Successfully Deleted message')
        # </editor-fold>
        ...
        # <editor-fold defaultstate="collapsed" desc="logging">
        logger.info(f'[{message.chat.id} ({message.message_id})] Successfully informed about mute.')
        logger.info(f'[{message.chat.id} ({message.message_id})] Done.')
        # </editor-fold>
        return
    if match.group('count'):
        ...
        if match.group('unit') in ('m', 'min', 'minute', 'minutes'):
            ...
            # <editor-fold defaultstate="collapsed" desc="logging">
            logger.info(f'[{message.chat.id} ({message.message_id})] unit of term is invalid; ignoring...')
            logger.info(f'[{message.chat.id} ({message.message_id})] Done.')
            # </editor-fold>
            return
